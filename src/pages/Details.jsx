...
import { FaArrowLeft, FaStar, FaMapMarkerAlt, FaBed, FaBath, FaRulerCombined, FaCouch, FaHeart } from "react-icons/fa";
...
const Details = () => {
  ...
  const property = useMemo(() => {
    ...
    return {
      ...
      features: [
        { icon: "fas fa-bed", text: `${found.rooms} Chambre${found.rooms > 1 ? "s" : ""}` },
        { icon: "fas fa-bath", text: `${found.baths} Salle${found.baths > 1 ? "s de bain" : " de bain"}` },
        { icon: "fas fa-ruler-combined", text: `${found.area} m²` },
        { icon: "fas fa-couch", text: found.furnished ? "Meublé" : "Non meublé" },
      ],
      equipments: [
        ...(found.wifi ? [{ icon: "fas fa-wifi", text: "WiFi haut débit" }] : []),
        ...(found.ac ? [{ icon: "fas fa-fan", text: "Climatisation" }] : []),
        ...(found.parking ? [{ icon: "fas fa-car", text: "Parking privé" }] : []),
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="relative overflow-hidden rounded-lg">
              <img src={currentMainImage} alt={property.title} className="w-full h-[400px] lg:h-[500px] object-cover" />
            ...
            <div className="grid grid-cols-2 gap-4">
              {property.secondaryImages.map((url, i) => (
                <div key={i} className="relative overflow-hidden rounded-lg cursor-pointer hover:opacity-80 transition" onClick={() => handleThumbnailClick(url)}>
                  <img src={url} alt="" className="w-full h-[190px] lg:h-[240px] object-cover" />
                  {i === 3 && (
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <span className="inline-block bg-blue-600 text-white px-3 py-1 rounded-full text-sm font-semibold mb-3">{property.type}</span>
                  <h1 className="text-3xl font-bold text-gray-800 mb-2">{property.title}</h1>
                  <div className="flex items-center text-gray-600 mb-3">
                    <FaMapMarkerAlt className="mr-2" /> <span>{property.location}</span>
                  </div>
                ...
                <div className="text-right">
                  <div className="text-blue-600 font-bold text-3xl">{property.price.toLocaleString()} FCFA</div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <p className="text-gray-700 leading-relaxed">
                Magnifique {property.type.toLowerCase()} à {property.location}. {property.rooms} chambre{property.rooms > 1 ? "s" : ""}, {property.baths} salle{property.baths > 1 ? "s de bain" : " de bain"}, {property.area} m². {property.furnished ? "Meublé et équipé." : "À aménager."}
              </p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-2xl font-bold text-gray-800">{property.rating}</span>
                  <span className="text-gray-500">({property.reviews} avis)</span>
                </div>
              </div>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">Il y a {i + 2} mois</div>
                        </div>
                      </div>
                      <div className="flex items-center space-x-1">
                        {[...Array(5)].map((_, s) => (
                          <FaStar key={s} className={`text-sm ${s < 5 - i ? "text-yellow-500" : "text-gray-300"}`} />
                        ))}
                      </div>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-1">
            <ReservationSidebar price={property.price} period={property.period} />
          </div>
        </div>
      </div>
    </main>
